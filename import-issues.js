...
(async () => {
  const Storage = {
    set(key, value) {
      try { localStorage.setItem(key, JSON.stringify(value)); } catch (e) { console.warn('保存失败:', e.message); }
    }
  };
  ...
  // 导入 ST 议题
  Storage.set('dste_issues_v1_ST', data.st);
  console.log('✅ ST 议题导入成功:', data.st.length, '条');

  // 导入 AT 议题
  Storage.set('dste_issues_v1_AT', data.at);
