...
test.describe('Roadmap 页面', () => {
  ...
  test('显示真实版本数据（v0.3.3 最新）', async ({ page }) => {
    // 时间线轴应显示最新版本
    const timeline = page.locator('.card', { hasText: /v0\.3\.3/ }).first();
    await expect(timeline).toBeVisible();
  });

  test('版本详情卡片显示 CHANGELOG 内容', async ({ page }) => {
    // 应能看到 v0.3.3 的变更列表
    const versionCard = page.locator('[id="version-v0.3.3"]');
