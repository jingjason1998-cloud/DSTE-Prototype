...
test.describe('Roadmap 页面', () => {
  ...
  test('执行摘要 KPI 显示当前版本', async ({ page }) => {
    ...
    // roadmap-data.json 最新版本为 v0.5.0
    await expect(summary).toContainText('当前版本');
    await expect(summary).toContainText('v0.5.0');
  });

  test('显示真实版本数据（v0.5.0 最新）', async ({ page }) => {
    // 纵向时间线应显示最新版本
    const timeline = page.locator('.roadmap-timeline-vertical');
    await expect(timeline).toContainText('v0.5.0');
  });

  test('版本详情卡片显示 CHANGELOG 内容', async ({ page }) => {
    // 应能看到 v0.5.0 的变更列表
    const versionCard = page.locator('[id="version-v0.5.0"]');
