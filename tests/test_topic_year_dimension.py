"""
DSTE 战略专题年份维度测试
验证专题按年份管理的数据模型和UI展示
TDD: 先写测试，再写实现
"""

import re
from pathlib import Path

PROJECT_ROOT = Path("/Users/jasonjing/DSTE-Prototype")
SRC = PROJECT_ROOT / "src"


def _strategy_section():
    """提取战略专题管理相关代码（共享工具 + renderStrategyTopicsPage + 弹窗函数）"""
    content = (SRC / "cockpit.html").read_text(encoding="utf-8")
    start_marker = "// ===== 战略洞察与专题共享工具函数"
    end_marker = "// ===== 业务专题管理页面 ====="
    if start_marker not in content or end_marker not in content:
        return ""
    start = content.find(start_marker)
    end = content.find(end_marker, start)
    return content[start:end]


def _topics_section():
    """提取 siDefaultTopics 函数体"""
    content = (SRC / "cockpit.html").read_text(encoding="utf-8")
    marker = "function siDefaultTopics()"
    if marker not in content:
        return ""
    start = content.find(marker)
    rest = content[start:]
    # 找函数体结束位置（下一个同层级函数）
    end_match = re.search(r"\n      function ", rest[1:])
    end = start + 1 + end_match.start() if end_match else len(content)
    return content[start:end]


def _insights_page_section():
    """提取 renderInsightsPage 函数体"""
    content = (SRC / "cockpit.html").read_text(encoding="utf-8")
    marker = "function renderInsightsPage()"
    if marker not in content:
        return ""
    start = content.find(marker)
    rest = content[start:]
    end_match = re.search(r"\n      (function |// ===== )", rest[1:])
    end = start + 1 + end_match.start() if end_match else len(content)
    return content[start:end]


# ========== 数据模型测试 ==========

def test_topic_has_year_field():
    """专题数据对象必须包含 year 字段"""
    topics_section = _topics_section()
    # 在 siDefaultTopics 的初始数据中，每条专题应有 year 字段
    assert "year:" in topics_section, "专题初始数据缺少 year 字段"
    # 至少有一条数据的 year 不为空
    year_matches = re.findall(r"year:\s*(\d{4})", topics_section)
    assert len(year_matches) >= 1, "专题初始数据中未找到有效的 year 值"


def test_topic_year_values_reasonable():
    """专题年份值应在合理范围内（2020-2030）"""
    topics_section = _topics_section()
    year_matches = re.findall(r"year:\s*(\d{4})", topics_section)
    for year in year_matches:
        y = int(year)
        assert 2020 <= y <= 2030, f"专题年份 {y} 不在合理范围 2020-2030"


def test_storage_key_upgraded_to_v2():
    """专题存储键应升级到 v2 以支持 year 字段"""
    insights_section = _strategy_section()
    assert "dste_strategy_topics_v2" in insights_section or "SI_STORAGE_KEY_TOPICS" in insights_section, "专题存储未升级到 v2 键名"


def test_v1_migration_logic_exists():
    """应有 v1 到 v2 的迁移逻辑，自动为旧数据添加 year 字段"""
    insights_section = _strategy_section()
    # 迁移逻辑应检测 v1 数据
    assert "dste_strategy_topics_v1" in insights_section or "SI_STORAGE_KEY_TOPICS_LEGACY" in insights_section, "缺少 v1 数据检测逻辑"
    # 迁移后应删除 v1 数据
    assert "SI_STORAGE_KEY_TOPICS_LEGACY" in insights_section, "迁移后未清理 v1 数据"
    assert ("DSTE.Storage.remove" in insights_section and "SI_STORAGE_KEY_TOPICS_LEGACY" in insights_section) or "removeItem" in insights_section, "迁移后未清理 v1 数据"


def test_migration_infers_year_from_startDate():
    """迁移时应根据 startDate 推断年份，无日期默认当年"""
    insights_section = _strategy_section()
    # 应有从 startDate 提取年份的逻辑
    assert "startDate" in insights_section, "迁移逻辑未使用 startDate"
    # 应有默认年份处理（new Date().getFullYear() 或硬编码）
    assert "getFullYear" in insights_section or "2026" in insights_section, "迁移逻辑缺少默认年份"


# ========== UI 测试 ==========

def test_year_selector_exists():
    """专题列表Tab应包含年份选择器"""
    insights_section = _strategy_section()
    # 年份选择器应为 select 元素或按钮组
    assert "year-selector" in insights_section or "data-year" in insights_section, "缺少年份选择器"
    # 应有年份选项（2024-2028）
    assert "2024" in insights_section or "2025" in insights_section or "2026" in insights_section, "年份选择器缺少选项"


def test_topics_grouped_by_year():
    """专题列表应按年份分组展示"""
    insights_section = _strategy_section()
    # 应有按年份分组的渲染逻辑
    assert "yearGroups" in insights_section or "按年份" in insights_section, "缺少按年份分组逻辑"
    # 或者通过检查是否有年份标题来判断
    year_header_pattern = re.compile(r"\d{4}[^\d]*年|year.*header|group.*year", re.IGNORECASE)
    assert year_header_pattern.search(insights_section), "未找到年份分组标题的渲染逻辑"


def test_year_filter_affects_topic_table():
    """切换年份应过滤专题表格数据"""
    insights_section = _strategy_section()
    # 应有 filter 逻辑按 year 筛选
    assert "filter" in insights_section and "year" in insights_section, "缺少按年份过滤逻辑"
    # 或者通过检查 onchange/onclick 事件处理
    assert "onchange" in insights_section or "switchYear" in insights_section or "siFilterTopicsByYear" in insights_section, "年份切换缺少事件处理"


def test_kpi_stats_filtered_by_year():
    """KPI统计卡片应按当前选中年份过滤"""
    insights_section = _strategy_section()
    # KPI统计应在过滤后的数据上计算
    # 检查是否有 currentYear 或 selectedYear 变量参与统计计算
    assert "currentYear" in insights_section or "selectedYear" in insights_section, "KPI统计未使用年份过滤变量"


def test_new_topic_form_has_year_field():
    """新建专题表单应包含年份选择字段"""
    insights_section = _strategy_section()
    # 新建专题按钮或表单应有年份输入
    assert "btn-new-topic" in insights_section, "缺少新建专题按钮"
    # 由于新建专题可能是弹窗形式，检查是否有年份相关的表单元素
    # 至少应有 year 在表单相关代码中被引用
    form_section = insights_section.split("btn-new-topic")[1] if "btn-new-topic" in insights_section else insights_section
    assert "year" in form_section, "新建专题表单缺少 year 字段"


# ========== 回归测试 ==========

def test_insights_tab_not_affected_by_year():
    """洞察视图页面不应被年份选择器影响"""
    insights_page = _insights_page_section()
    # 年份选择器应只在专题管理页面中
    assert "year-selector" not in insights_page, "洞察视图不应包含年份选择器"


def test_existing_topic_fields_preserved():
    """现有专题字段应完整保留，year 是新增字段"""
    topics_section = _topics_section()
    # 检查原有字段仍然存在（v1.2 移除 priority/budget/actualCost）
    required_fields = ["id:", "name:", "type:", "dimension:", "status:", "owner:", "progress:"]
    for field in required_fields:
        assert field in topics_section, f"专题数据缺少原有字段 {field}"


def test_v2_fields_exist():
    """v1.2 新增字段应存在"""
    topics_section = _topics_section()
    new_fields = ["members:", "kmsUrl:", "researchObjectives:", "deliverables:", "previousTopicId:", "nextTopicId:"]
    for field in new_fields:
        assert field in topics_section, f"v1.2 专题数据缺少新增字段 {field}"


def test_deprecated_fields_removed():
    """v1.2 应移除 priority/budget/actualCost 字段"""
    topics_section = _topics_section()
    deprecated = ["priority:", "budget:", "actualCost:"]
    for field in deprecated:
        assert field not in topics_section, f"v1.2 专题数据仍包含已移除字段 {field}"
