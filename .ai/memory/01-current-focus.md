...
**临时 CAS 绕过（2026-07-29）**：帆软通行证登录成功后不回跳 DSTE，用户无法进入系统。已临时把 `dste.fineres.com` 加入 `isLocalDev` 白名单（`index.html` / `cockpit.html` / `business-topics.html` / `meetings.html` / `requirement-pool.html` / `rule-engine.html`），实现本地快速登录。已手动部署到生产。**后续必须恢复**，恢复前生产环境无 CAS 认证。建议单独排期 v0.7.17 热修并验证 CAS 回跳链路。
