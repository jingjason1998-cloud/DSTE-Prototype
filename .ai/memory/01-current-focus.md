...
## 状态
**v0.7.35 已发布**（2026-08-21 Kimi 会话，commit `b0110e6` + bump `d8dc60a`，tag `v0.7.35`）：RFC-011 P1 四项全部落地——① `AIClient.runToolLoop` 多轮工具调用（maxToolRounds=4 末轮强制收尾），callWithTools 与 GlobalAiDrawer 手写流式工具轮统一共用，chat 加 `userMessageHidden`；② queryMeeting* 工具优先读 Worker KV `dste_meetings_v1` 权威数据、context.meeting 回退、返回 `source` 字段；③ Worker `GET /api/ai/stats` 聚合 telemetry KV（成功率/P95/errorType/端点/趋势/反馈）+ cockpit 观测页 `#admin/ai-stats`（hash 直达，**侧边栏入口待并行会话 config.js 改动落地后补登**）；④ `scripts/ai-smoke-eval.mjs` 10 用例真实 Kimi 事故回归集（发版前手动跑）。验证：unit 624 / pytest 221 / lint 0 / check:scope ✓ / build ✓ / AI+导航 E2E 40/40；Worker 部署 Version `bb2765e9`，smoke eval 生产 10/10。**RFC-011 全部完成（P0 随 v0.7.34，P1 随 v0.7.35），P2 方向登记在 RFC（模型路由/会话服务端化/ai-stats 侧边栏入口）。**

**v0.7.34 已发布**（2026-08-21 Kimi 会话，commits `b5a0765`(RFC-010) + `3e276f0`(RFC-011) + bump `8924a5f`，tag `v0.7.34`）：① RFC-010 工作区页签 keep-alive 随版上线（此前未提交）；② RFC-011 AI 助手稳定性 P0 五项——Worker 错误响应带 errorType/upstreamStatus 不再一律 502、前端按分类给可操作提示、invalid_request 自动清空会话重试（杜绝坏历史永久 502）、内部超时不再被当用户取消静默吞（独立 AbortController + 超时重试 1 次，Worker 29s×4→25s×2 对齐 60s 预算）、temperature 透传（**实测发现 kimi-k2.7-code-highspeed 只允许 temperature=1**，Worker 对 invalid temperature 自动摘字段兜底）、营销预算 AI 分析恒「未返回有效内容」修复（Worker 返回 choices 嵌套，原读 res.content 恒空且污染缓存）、👍👎 反馈真实上报 /api/ai/log + promptHash 修复（原恒 h_0）；③ 提示词收敛：删两处死代码/漂移副本，TopicAiChat 接共享库。验证：unit 622 / pytest 221 / lint 0 / check:scope ✓ / build ✓ / AI E2E 18/18；Worker 已 wrangler 部署（Version `17f34a48`）+ 生产 curl 验证 errorType 透传与 temperature 兜底均正常。**教训：fetchWithRetry 对可重试状态码耗尽后抛自己包装的 Error（response 挂在 .response 上），调用方必须回退解析 err.response 否则拿不到响应体。**详见 session-log 顶部条目。
...
- **会议材料审核 Worker 迁移（方案 B）**：已登记 T080，暂缓。
...
- **AI 交互 UI 升级（对标 Kimi 网页版，已随 v0.7.2 发布）**：
  ...
  - 验证：build / check:scope ✓、unit 466 ✓、E2E 60 ✓、视觉走查 ✓
- **会议材料审核修复（已发布 commit `00869bc`）**：v0.6.7 把 nginx `/api/` 切到 Worker 致审核 7 端点 404；`scripts/update-nginx-api-proxy.sh` 改为按路径分流（审核端点回 Flask 8766，其余走 Worker），生产已验证；**遗留**：Flask `KMS_API_TOKEN` 失效，需服务器更新 `/opt/meeting-reviewer/src/.env` 并重启 meeting-reviewer
- **方案 B 登记**：`.ai/tasks/active/T080-review-worker-migration.md`（审核端点移植 Worker 的完整契约），暂缓实施
- 注意：pytest `test_score_color_rules` 失败为并行会话 todo-panel 改动所致，非本会话
