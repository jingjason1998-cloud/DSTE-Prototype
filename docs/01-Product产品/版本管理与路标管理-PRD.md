...
```json
{
  ...
  "frontend": {
    "version_tag": "v0.4.10",
    "cockpit_html": { "md5": "1460b4b4...", "mtime": "2026-06-15 08:07:04", "size": 148202 },
...
| # | 文件 | 修改内容 |
|---|------|---------|
| 1 | `src/lib/config.js` | 系统管理分组新增"版本审计"入口 |
| 2 | `src/cockpit.html` | 注册 `dashboard/version-audit`；`renderDevTimeline` 改为读取 `roadmap-data.json` |
| 3 | `src/cockpit.html` | 新增 `renderVersionAudit()` 函数 |
| 4 | `proxy_server.py` | 新增 `/api/version-audit` 端点 |
| 5 | `scripts/generate-roadmap-data.js` | 确保稳定生成 `roadmap-data.json` |
| 6 | `.github/workflows/update-roadmap.yml` | tag push 时触发自动生成 |
| 7 | `docs/01-Product产品/开发计划.yml` | 新增/维护开发计划数据源 |
| 8 | `tests/e2e/version-audit.spec.js` | 新增版本审计 E2E 测试 |
...
| 优先级 | 缺口 | 说明 | 计划版本 |
|-------|------|------|---------|
| **高** | **数据未从 JSON 加载** | `renderDevTimeline()` 当前使用硬编码数据，未读取 `src/data/roadmap-data.json` | v0.5.0 |
| **高** | **数据不同步** | `src/data/roadmap-data.json` 有 17 个版本（到 v0.4.10），页面只显示 9 个版本（到 v0.4.0） | v0.5.0 |
| **高** | **版本审计看板未实现** | RFC-005 已设计，但无代码 | v0.5.0 |
