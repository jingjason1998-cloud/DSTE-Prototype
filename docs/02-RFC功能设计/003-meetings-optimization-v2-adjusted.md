# RFC-003-Adj: 经营分析会模块优化方案 v2.0 — 调整版（残缺稿）

> **⚠️ 文件残缺（2026-08-21 注记）**：本文档的**标题、正文与附录 A/B 已丢失**（git 历史显示在 v0.4.9 之前即已残缺），现存内容仅为**附录 C/D/E**。正文所述优化方案的完整功能设计可参考 [经营分析会-功能设计文档](../01-Product产品/经营分析会-功能设计文档.md)。
>
> **实现迁移注记**：下文以 `renderMeetings()`、`exe/meetings/edit` 路由、`window._meetingsData` 描述的 cockpit 内实现，已整体迁移至独立页面 `src/meetings.html` + `src/meetings/` 模块群。另「至少保留 1 项议程」规则已于 **v0.7.30 移除**（议程可删到 0 条）。
...
场景指标卡与筛选栏「会议场景」下拉框完全一致，采用业务场景命名：

| 场景 key | 显示名称 | 图标 | 标识色 | 说明 |
|----------|---------|------|--------|------|
| `union_quarterly` | 片联季度会议 | 🏛️ | `#8b5cf6` 紫 | 片联层级季度经营分析会 |
| `hq_routine` | 营销本部月/双周会 | 📅 | `#3b82f6` 蓝 | 营销本部月度/双周经营例会 |
| `region_routine` | 战区月度经营分析会 | 🗺️ | `#0ea5e9` 青 | 各战区月度经营分析会 |
| `lagging_region` | 落后战区业绩承诺会 | 🚨 | `#dc2626` 红 | 落后战区业绩承诺会暨经分会 |
| `lagging_vertical` | 落后垂直客群经分会 | ⚠️ | `#ef4444` 红 | 落后垂直客群经营分析会 |

> **注意**：不再设置「其他」场景卡。全部会议必须归属以上五类场景之一，由 `SCENARIO_CONFIG` 统一管理。`SCENE_CARD_CONFIG` 与 `filter-scenario` 下拉选项保持 key、名称、图标、颜色完全一致。
...
// SCENE_CARD_CONFIG 与 SCENARIO_CONFIG 保持一致的 key 映射
const KNOWN_SCENES = ['union_quarterly', 'hq_routine', 'region_routine', 'lagging_region', 'lagging_vertical'];
const sceneCounts = KNOWN_SCENES.reduce((acc, key) => {
  acc[key] = meetings.filter(m => m.scenario === key).length;
  return acc;
}, {});
```
...
- margin-bottom: 10px
  ...
  - ✏️ 编辑按钮（可选，hover 时显示）：font-size: 11px, padding: 2px 8px, border-radius: 4px, background: var(--primary-light), color: var(--primary), cursor: pointer
    - 点击编辑按钮：阻止事件冒泡，导航到独立编辑页面 `exe/meetings/edit`，不打开主视图
    - 编辑按钮使用 `data-edit-id` 属性存储会议 ID，通过事件委托处理点击
...
- align-items: center
  ...
  - ✏️ 编辑按钮（hover 时显示或常驻小图标）：点击导航到独立编辑页面
```

**卡片点击行为区分**：
- 点击卡片空白区域/标题/元信息：打开【会议主视图】（`openMeetingDetail`）
- 点击 ✏️ 编辑按钮：阻止冒泡，设置 `window._editMeetingId = id`，导航到 `exe/meetings/edit`
...
打开【会议主视图】（全屏沉浸式，非弹窗）
  ...
  保持主视图打开（背景遮罩）
        ↓
  打开【会议编辑页面】（全屏沉浸式，独立路由 `exe/meetings/edit`）
```

#### D.1a 独立编辑页面路由

**路由定义**：`exe/meetings/edit`

**页面渲染函数**：`renderMeetingEditPage()`

**数据传递**：
- 全局变量 `window._editMeetingId` 存储待编辑会议的 ID
- `renderMeetingEditPage()` 从 `window._meetingsData` 中查找对应会议
- 若 `window._editMeetingId` 为 `null` 或会议不存在，自动回退到 `exe/meetings`

**与弹窗编辑的关系**：
- 会议列表卡片的编辑按钮 → 独立编辑页面（本设计）
- 会议主视图内的编辑按钮 → 可保留弹窗编辑或统一跳转独立页面（二选一，建议统一）
- 独立编辑页面与弹窗编辑复用同一套表单字段和保存逻辑
...
| 主持人 | text | | 最长 20 字 |
| 记录人 | text | | 最长 20 字 |
| 会议场景 | select | | 5 选项：片联季度会议 / 营销本部月/双周会 / 战区月度经营分析会 / 落后战区业绩承诺会 / 落后垂直客群经分会 |
| 层级 | select | | 3 选项：L1（片联）/ L2（营销本部/战区常规）/ L3（落后整改）|
...
| 删除议程项 | 移除当前项 | ~~至少保留 1 项，仅剩1项时删除按钮 disabled~~（该规则已于 v0.7.30 移除，议程可删到 0 条） |
| 上移 | 与上一项交换位置 | 第1项不可上移 |
...
window._editingMeeting = {
  ...
  host: "",
  recorder: "",
  scenario: "union_quarterly",    // 默认第一个场景，key 与 SCENARIO_CONFIG 一致
  level: "L1",    // L1=片联, L2=本部/战区常规, L3=落后整改(战区/垂直)
  status: "planned",
  meeting_link: "",
  agenda: [
...
- [ ] 会议链接格式非法时保存，显示错误提示
- [ ] 议程项可添加、删除（~~至少保留1项~~，v0.7.30 起可删到 0 条）、上移、下移
- [ ] 议程时长变更时总时长实时更新
...
- cursor: pointer
- 背景色按场景区分：
  - 片联季度会议：#8b5cf615 背景 + #8b5cf6 文字
  - 营销本部月/双周会：#3b82f615 背景 + #3b82f6 文字
  - 战区月度经营分析会：#0ea5e915 背景 + #0ea5e9 文字
  - 落后战区业绩承诺会：#dc262615 背景 + #dc2626 文字
  - 落后垂直客群经分会：#ef444415 背景 + #ef4444 文字
