...
> **历史快照（2026-05）**：本文记录的是 v0.4 时代 A 级架构升级完成时的状态，文中的页面结构、入口数量（4 个）、测试数量（pytest 30 + E2E 8）与"已知问题"均为**当时状态**，不代表当前系统现状。当前实际：vite.config.js 已注册 18 个构建入口，测试规模为 Vitest 600+ / pytest 211 / E2E 450+（约数，统计于 2026-08），reviewer.html 已引用 shell.css。现状说明请见 `architecture.md` 与 `tech-stack.md`。
