...
项目用 `.ai/memory/` 下的文件在 AI 会话之间传递上下文。**这套机制没有自动化，靠每个会话自觉执行。**

**会话开始时：**
1. 读 `.ai/memory/00-bootstrap.md`（技术事实与硬约束）
2. 读 `.ai/memory/01-current-focus.md`（当前开发焦点）
3. 中断恢复时再读 `.ai/memory/08-checkpoint.md`

**会话结束时（包括发布/救火会话，无一例外）：**
1. 更新 `.ai/memory/01-current-focus.md`：开发线状态、**生产版本号**、下一步
2. 在 `.ai/memory/06-session-log.md` 顶部追加本次会话摘要（主题/操作/修改文件/验证/状态）
3. 复杂任务中断时更新 `.ai/memory/08-checkpoint.md`

**判断标准：会话产生了 git 提交或版本发布，但 `.ai/memory/` 没有变化 = 没做完。**
