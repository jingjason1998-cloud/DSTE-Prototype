...
### Fixed
- **同步失败 toast 刷屏**：登录过期期间（生产 CAS 临时绕过无 `dste-token`）同步持续 401，队列积压到 100 条上限后，每次变更入队都弹一条「同步队列已满」错误提示，无任何去重导致页面叠十几条红色 toast。修复：`SyncQueue` 新增 `_notifyThrottled()` 节流（同一类提示 60 秒内只显示一次，`toastCooldownMs` 可配），「队列已满」「队列积压较多」「同步失败（按端点）」三条提示全部接入；「登录已过期」提示原本已有一次性保护，维持不变。单测补 3 个节流用例。注意：提示不再刷屏，但积压数据仍需恢复 CAS 认证重新登录后才会补传。
...
### Fixed
- **会议评估待办打不开评估弹窗**：人员目录 v4 迁移后 `host`/`recorder`/`agenda.owner` 变为 PersonRef 对象，而 `calculateAutoScore` 仍按字符串调用 `.trim()`，已完成会议点击「会议评估」待办即抛 `trim is not a function`，弹窗无法打开；新增 `personNameOf()` 兼容层，单测补 PersonRef 形态用例。
- **议程删除按钮显示问号、最后一条不可删**：`icon-mapping.js` 缺 `x` 键导致全站 20+ 处 `icon('x')` 兜底渲染为问号图标，补 `x: 'x'` 别名；按用户确认移除「至少保留一个议程项」保护（按钮 disabled + 函数 guard + 保存时「未设置议程」占位回填），议程可删到 0 条。
- **关联重点工作下拉重复**：`getOmpKeyWorks()` 未过滤周期与来源，种子数据 3 个年度周期 × 年度计划源头/OMP 派生双副本导致同名任务重复出现；与 OMP 重点工作列表口径对齐（仅当前周期、源头有派生副本时只保留副本、排除子任务）。
- **纪要定稿状态不可达**：原 `minutesStatus === 'final'` 无任何运行时入口（仅种子数据），UI「已定稿」永不显示；改为有纪要内容即视为已定稿——保存时 `minutesStatus` 置 `final` 并联动 `pipeline.minutesApproved`，存量数据由 `migrateMeetingsData()` 每次初始化自动修正；同时清理保存路径中 `hasMinutes`/`minutesStatus` 重复键被旧值覆盖的问题。

## [v0.7.29] - 2026-08-20

### Fixed
- **战略专题「下一年继续深化」按钮对所有状态可见**：原 `showDeepen` 条件要求专题状态为执行中/已闭环才显示深化入口，导致洞察/规划阶段的专题（如 2025 年「供应链」）无法发起年度深化；改为只要尚无下一年专题（`nextTopicId`）即显示，已深化的专题仍显示「查看下一年深化专题」。同步更新 `tests/e2e/strategy-topics.spec.js` 深化用例覆盖任意状态。
...
### Added
- **战略专题管理新增序号编号与排序功能**：列表视图新增「序号」首列，专题按年份分组从 1 自动编号；每行提供上移/下移按钮调整顺序，调整后同分组序号自动重排并持久化（含单条云端同步）；默认排序由「最近更新」改为「序号升序」，序号列表头支持点击切换排序方向；新建专题自动追加到同分组末尾；旧数据通过 `siMigrateTopicSeq()` 迁移自动补齐序号。
  - `tests/e2e/strategy-topics.spec.js` 新增 2 个用例（序号渲染与禁用态、下移交换并持久化）。
  - `tests/test_topic_crud.py` 新增 4 个结构断言。

## [v0.7.22] - 2026-07-31

### Fixed
- **修复旧 AI 会话中的重复 tool_call_id 导致持续 502**：将 AI 会话存储 key 从 `dste_ai_sessions_v1` 升级到 `dste_ai_sessions_v2`，丢弃可能被旧版本污染的历史会话；同时保留 v0.7.20 的每轮前缀机制，防止新会话再次产生重复 id。
...
### Fixed
- **修复会议卡片「纪要」标签显示「暂无纪要」但实际有内容**：会议卡片底部的 `renderTabs` 仅依据 `m.hasMinutes` 判断，未展示 `m.minutes_content` 实际内容；同时旧数据可能存在 `minutes_content` 有内容但 `hasMinutes` 标志未同步的情况。已做以下修复：
...
### Fixed
- **修复 iframe 嵌入页面无法向下滚动**：`assets/css/main.css` 中 `[data-embed="true"]` 模式将 `.app` / `.app-body` / `.content-area` 都设为 `overflow: visible` 与 `height: auto`，导致 iframe 内部失去滚动容器，内容超出时无法向下滚动。已恢复为 `.app { height: 100vh; overflow: hidden; }` 与 `.content-area { overflow-y: auto; }`，使外部页在 cockpit 工作区 iframe 内可正常内部滚动。
- **移除误加入的「营销线预算执行监控」导航与文件**：v0.7.6 中误将未完成的 `src/budget-execution-monitor.html`（2MB 独立页面）及对应 sidebar/config/AI 上下文注册一并发布。现已在 `src/lib/config.js` 与 `src/cockpit.html` 中移除相关入口，并删除该文件，避免生产环境出现无内容页面。
...
### Fixed
- **修复 nginx 审核路径正则误匹配 `/api/review-scores`**：`scripts/update-nginx-api-proxy.sh` 中 Flask 审核端点分流正则 `^/api/(review|batch|scenes|history|summary|config)` 缺少结束锚点，导致 `/api/review-scores` 也被错误路由到本机 Flask（Flask 无此端点，返回 404 HTML）。经营分析会页面加载时会调用该 Worker 端点拉取材料审核评分，从而报错 "load remote review scores failed"。现已将正则修正为 `^/api/(review|batch|scenes|history|summary|config)(/|$)`，仅把真正的审核端点及其子路径分流到 Flask，其余 `/api/` 请求继续走 Worker。

## [v0.7.4] - 2026-07-24

### Fixed
- **修复外部页 iframe 嵌入时双层 shell 嵌套**：`src/meetings.html`、`src/employee-directory.html`、`src/st-issue-tracking.html`、`src/at-issue-tracking.html` 未识别 `?embed=1`，导致被 cockpit 工作区 iframe 嵌入时仍渲染顶部导航与侧边栏，出现双层外壳。现已在上述页面统一增加嵌入模式检测，设置 `data-embed="true"` 并跳过 shell 渲染，同时向父窗口汇报高度实现 iframe 自适应。

### Changed
- **经营分析会列表布局改为 7:3**：`src/meetings.html` 主体网格由 `1fr 1fr`（50/50）调整为 `7fr 3fr`，会议内容区占 70%，待办提醒区占 30%，并保留响应式断点自动堆叠为单列。

### Added
- **新增 embed 模式 E2E 测试**：`tests/e2e/meetings-embed.spec.js` 与 `tests/e2e/external-pages-embed.spec.js` 覆盖 meetings、人员与组织管理、片联 ST/AT 议题跟踪表等外部页在 `?embed=1` 下隐藏导航外壳、内容区占满全宽的验证。
...
### Fixed
- **修复会议行动项进展说明刷新后丢失**：`src/meetings/data-store.js` 的 `migrateMeetingsData()` 在规范化 action 时遗漏 `progressNote`，每次刷新都会丢弃已保存的进展说明。

### Added
- **会议行动项跟进记录**：保存进展说明时自动追加到 `progressLogs`；`pending-actions` 抽屉新增「跟进记录」列表，按时间倒序展示；`meeting-detail` 行动项展示跟进记录条数徽标。

## [v0.6.16] - 2026-07-20

### Fixed
- **修复多处 SVG 图标显示为乱码**：将动态更新的标题/按钮从 `textContent` 改为 `innerHTML`，避免 `icon()` 返回的 SVG 字符串被当纯文本渲染。涉及待闭环行动抽屉标题、会前准备标题、批量送审按钮、议程重新送审按钮、需求池 AI 分析按钮、经营专题里程碑状态标签、OMP 分解行删除按钮。
...
### Fixed
- **年度计划新增重点工作不显示在战略执行**：`renderTasks()` 中「年度计划重点工作 → OMP 执行任务」的自动派生原先被 `!hasOmpTasks` 门挡住——仅当当前周期一个 OMP 任务都没有时才同步。已发布 / 已有其他 OMP 重点工作后再新增的源头任务永远不会被派生，而执行侧列表只认 `source==='omp'`，导致新增重点工作看不到。现改为每次进入战略执行页都幂等补齐未派生的源头任务（`omp_syncAnnualPlanTasksToExecution` 只补缺失项，无变化时不额外推送云端），新增即可见，无需再点「发布到执行阶段」。
...
### Fixed
- **修复「N 条变更正在同步到云端…」横幅一直不消失**：两个叠加原因——①`loadRemoteMeetings` 合并后调用 `persistMeetings()`，把刚从云端拉下来的数据又当变更反向重新入队推送，每次加载都产生大量 pending；②横幅在队列同步完成后不自动刷新，停在旧数字。现改为：加载只推「本地比云端新」的既有记录、云端数据不再反推（本地独有记录仍由「未同步」横幅确认上传）；横幅在有活跃同步时每 3 秒轮询刷新、同步完成自动清除。
- **横幅区分「登录过期卡住」与「正在同步」**：`SyncQueue.getStatus()` 新增 `authPending` 统计；登录过期导致的待同步项显示红色「登录已过期，重新登录后自动补传」，而非误导性的「正在同步」。
