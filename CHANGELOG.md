...
### Added
- **新增预算执行监控独立页面（仅关联不操作）**：
  - 新增 `src/budget-execution-monitor.html` 及 `src/pages/budget-execution-monitor/{main.js,style.css}`，作为预算执行项与业务专题、重点工作的纯关联入口。
  - 预算执行项以种子数据只读展示（首次访问写入，用户清空后不再自动恢复；后续可替换为外部数据源）。
  - 被关联的业务专题、重点工作在本页只读展示，点击后通过深链跳转各自原生页面操作。
  - 新增关联选择器弹窗，支持按业务专题 / 重点工作搜索并多选关联；关联数据以快照形式存入 `dste_budget_associations_v1`。
...
- **业务专题与 OMP 重点工作支持深链打开**：
  ...
  - `src/cockpit.html` 支持 `?taskId=xxx&modal=view|edit#exe/tasks` 参数，自动打开重点工作详情/编辑弹窗。
